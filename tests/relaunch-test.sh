...
EOF
  ...
  [[ "$luaout" == *"boot --quiet"* ]] || fail "registration must carry the boot command: $luaout"
...
# Migration is exact on BOTH lines. Delegating to the generic block remover
# accepted any marker PREFIX and deleted a following line that merely
# CONTAINED "relaunch boot", destroying user comments.
obreset
...
EOF
obrun ensure-hooks >/dev/null 2>&1
grep -q 'omarchy-relaunch notes about the plugin' "$OB/autostart.lua" \
  || fail "migration deleted a user comment beginning with the marker text"
grep -q 'reminder: relaunch boot is hidden' "$OB/autostart.lua" \
  || fail "migration deleted a comment merely containing the hook words"
grep -qxF -- '-- omarchy-relaunch (managed; hidden from the Relaunch list)' "$OB/autostart.lua" \
  || fail "a marker NOT followed by our hook must be left alone"
grep -q 'brave' "$OB/autostart.lua" || fail "migration ate an unrelated launch line"
grep -q 'sunsetr' "$OB/autostart.lua" || fail "migration ate an unrelated launch line"
...
# --force overrides an existing marker.
obguard sig-A boot --force >/dev/null 2>&1
jq -e '.outcome == "launched"' "$OB/cfg/last-boot.json" >/dev/null \
  || fail "--force must run despite the guard"
# …and --force as a session's FIRST boot must still record completion, or the
# next ordinary boot duplicates the whole restore.
rm -rf "$OB/run/omarchy-relaunch"
obguard sig-FORCE boot --force >/dev/null 2>&1
[[ -e "$OB/run/omarchy-relaunch/boot-sig-FORCE.done" ]] \
  || fail "--force must write the completion marker so a later boot suppresses"
out="$(obguard sig-FORCE boot 2>&1)"
[[ -n "$out" ]] || fail "an ordinary boot after --force must be suppressed"
...
# A crashed first boot must not lock out every retry, and the mechanism that
# matters is FD inheritance: `exec {fd}>` has no close-on-exec, so a launched
# application inherits the flock and holds it for the whole session. Kill the
# boot PARENT while a descendant is still alive and running, then require the
# next boot to run anyway.
rm -rf "$OB/run/omarchy-relaunch"
rm -f "$OB/cfg/last-boot.json"
cat >"$OB/cfg/config.json" <<'EOF'
{"staggerSeconds":0,"ignored":[],"entries":[
  {"class":"hangs","workspace":1,"exec":"sleep 137","execSource":"guess","enabled":true}
]}
EOF
env RELAUNCH_CONFIG_DIR="$OB/cfg" RELAUNCH_AUTOSTART="$OB/autostart.lua" \
  RELAUNCH_HYPRLAND_LUA="$OB/hyprland.lua" RELAUNCH_HYPR_CONF="$OB/hyprland.conf" \
  RELAUNCH_PLUGIN_DIR="$OB/plugin" HYPRCTL="$HYPRCTL_STUB" FAKE_CLIENTS="$FAKE_CLIENTS" \
  RELAUNCH_VERIFY_SLEEP=9 RELAUNCH_VERIFY_DEADLINE=0 RELAUNCH_VERIFY_INTERVAL=0 \
  XDG_RUNTIME_DIR="$OB/run" HYPRLAND_INSTANCE_SIGNATURE=sig-C \
  "$RELAUNCH" boot >/dev/null 2>&1 &
hangpid=$!
sleep 1
kill -9 "$hangpid" 2>/dev/null || true
wait "$hangpid" 2>/dev/null || true
[[ ! -e "$OB/run/omarchy-relaunch/boot-sig-C.done" ]] \
  || fail "a killed boot must not have written a completion marker"
# The launched `sleep 120` is still alive and inherited the fd unless we
# closed it. Retry must not be blocked by that descendant.
rm -f "$OB/cfg/last-boot.json"
RELAUNCH_BOOT_LOCK_WAIT=5 obguard sig-C boot >/dev/null 2>&1
jq -e '.outcome == "launched"' "$OB/cfg/last-boot.json" >/dev/null \
  || fail "a killed boot must not lock out retries via an inherited lock fd"
pkill -f 'sleep 137' 2>/dev/null || true
cat >"$OB/cfg/config.json" <<'EOF'
{"staggerSeconds":0,"ignored":[],"entries":[
  {"class":"guarded","workspace":1,"exec":"true","execSource":"guess","enabled":true}
]}
